...
const PricingContentfulTemplate = ({ data, location, pageContext }) => {
  const pricing = data.contentfulPricing
  ...
  return (
    <Layout location={location} title={siteTitle}>
      <SEO
        title={pricing.title}
        description={`${siteTitle} - Cennik - ${pricing.title}`}
      />
      <Section maxWidth={maxWidth}>
        ...
        <ContentfulPricingText>
          {documentToReactComponents(pricing.description.json, options)}
        </ContentfulPricingText>
        <p>
          Cena: {pricing.price}zł / {pricing.time}min
        </p>
        <hr
          style={{
...
export default PricingContentfulTemplate

export const pricingPageQuery = graphql`
  query ContentfulPricingBySlug($slug: String!) {
    site {
      siteMetadata {
        title
      }
